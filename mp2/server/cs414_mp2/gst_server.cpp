...
void GstServer::initPipeline(GstData *data, int videoPort, int audioPort) {
	...
	g_object_set (data->videoUdpSink, "host", (data->clientIp == "127.0.0.1" ? "localhost" : data->clientIp), "port", videoPort, NULL);
	g_object_set (data->audioUdpSink, "host", (data->clientIp == "127.0.0.1" ? "localhost" : data->clientIp), "port", audioPort, NULL);
